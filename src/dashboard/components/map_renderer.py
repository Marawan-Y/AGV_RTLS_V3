...
class MapRenderer:
    ...
    def __init__(
        self,
        plant_map_path: str = "assets/plant_map.png",
        zones_path: str = "data/settings/zones.geojson",
    ):
    ...
    def create_base_figure(self, title: str = "Plant Map") -> go.Figure:
        ...
        # Set layout with equal aspect ratio (so polygons don’t skew)
        fig.update_layout(
            title=title,
            xaxis=dict(
                title="X Position (m)",
    ...
    # ----------------------------
    # Zones (GeoJSON-first; fallback to ZoneManager)
    # ----------------------------
    @st.cache_data(show_spinner=False)
    def _load_geojson(_self) -> Dict[str, Any]:
        """
        Load zones GeoJSON once (cached).
        Returns FeatureCollection {} if file missing or invalid.
        """
        try:
            if not _self.zones_path.exists():
                logger.info(f"Zones GeoJSON not found at {_self.zones_path}")
                return {"type": "FeatureCollection", "features": []}
            txt = _self.zones_path.read_text(encoding="utf-8")
            data = json.loads(txt)
            if not isinstance(data, dict) or "features" not in data:
                logger.warning("Zones GeoJSON is not a valid FeatureCollection.")
                return {"type": "FeatureCollection", "features": []}
            return data
        except Exception as e:
    ...
    def _draw_ring(
        ...
        """Draw a single polygon ring (outer) in plant coordinates."""
    ...
    def _render_geojson_zones(
        self,
        fig: go.Figure,
        show_labels: bool,
        highlight_zones: Optional[List[str]],
    ) -> bool:
        """
        Render zones from GeoJSON. Returns True if at least one zone rendered.
        Expected geometry: Polygon/MultiPolygon with plant coordinates [[x, y], ...].
        """
        gj = self._load_geojson()
        ...
        for feature in features:
            ...
            name = props.get("name") or props.get("id") or "Zone"
            category = props.get("category", "GENERIC")

            # allow per-feature styling
            fill_color = props.get("color", "rgba(0, 120, 255, 0.15)")
            line_color = props.get("line_color", "rgba(0, 120, 255, 0.85)")

            if highlight_zones and name in highlight_zones:
                # emphasize
                fill_color = props.get("color_highlight", "rgba(255, 165, 0, 0.25)")
                line_color = props.get("line_color_highlight", "rgba(255, 140, 0, 0.95)")
            ...
            try:
                if gtype == "Polygon":
                    ...
                    if coords and coords[0]:
                        outer = coords[0]  # draw only outer to keep logic simple/reliable
                        self._draw_ring(fig, outer, name, fill_color, line_color, show_labels)
                        rendered = True
                elif gtype == "MultiPolygon":
                    # coords: [ [ [ outer ], [hole]... ],  [ [ outer ], ... ], ... ]
                    for poly in coords:
                        if poly and poly[0]:
                            outer = poly[0]
                            self._draw_ring(fig, outer, name, fill_color, line_color, show_labels)
                ...
                logger.error(f"Error rendering GeoJSON zone '{name}': {e}")
    ...
    def add_zones(
        ...
        highlight_zones: Optional[List[str]] = None,
    ) -> go.Figure:
        """
        Add zone boundaries to figure.
        - Primary source: GeoJSON at data/settings/zones.geojson (plant x/y).
        - Fallback: legacy ZoneManager if GeoJSON is missing/empty.
        """
        # Try GeoJSON first
        got_any = self._render_geojson_zones(fig, show_labels, highlight_zones)
        if got_any:
            return fig

        # Fallback to legacy DB-stored zones (existing behavior)
        ...
        zone_colors = {
            ...
            "GENERIC": "rgba(0,120,255,0.15)",  # default-ish
        }
        ...
        for zone_id, zone_info in zones.items():
            if "vertices" in zone_info and zone_info["vertices"]:
                try:
                    ...
                    is_highlighted = bool(highlight_zones and zone_id in highlight_zones)
                    ...
                    # Add zone label
                    if show_labels and zone_info.get("centroid_x") is not None:
                        fig.add_annotation(
    ...
    def add_agv_positions(
        ...
        for agv_type in positions["type"].unique() if "type" in positions.columns else ["Unknown"]:
            type_positions = (
                positions[positions["type"] == agv_type]
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        ...
                        angle=type_positions["heading_deg"]
                        if show_heading and "heading_deg" in type_positions.columns
                        else 0,
                        line=dict(color="white", width=2),
                    ),
                    text=type_positions["agv_id"]
                    if show_labels and "agv_id" in type_positions.columns
                    else None,
                    ...
                    customdata=type_positions[["speed_mps", "battery_percent"]].values
                    if all(col in type_positions.columns for col in ["speed_mps", "battery_percent"])
                    else None,
                )
            )
    ...
    # ----------------------------
    # Trajectory / overlays / alerts / animation
    # ----------------------------
    def add_trajectory(
        self,
        fig: go.Figure,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                name="Start/End",
                marker=dict(size=[15, 15], color=["green", "red"], symbol=["circle", "square"]),
                showlegend=False,
            )
        )
    ...
    def add_heatmap_overlay(self, fig: go.Figure, heatmap_data: Dict, opacity: float = 0.5) -> go.Figure:
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                opacity=opacity,
                showscale=True,
                colorbar=dict(title="Density", x=1.02, thickness=15),
                hoverinfo="skip",
            )
        )
    ...
    def create_animated_map(
        self, positions_over_time: List[pd.DataFrame], timestamps: List[datetime]
    ) -> go.Figure:
        ...
        for i, (positions, timestamp) in enumerate(zip(positions_over_time, timestamps)):
            ...
            frames.append(go.Frame(data=frame_data, name=str(i), traces=[0]))
